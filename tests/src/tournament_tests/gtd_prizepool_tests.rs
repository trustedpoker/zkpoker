...
fn test_winner_basic_gtd_prizepool() {
    ...
    let admin = test_env.create_admin_user("Admin".to_string());

    let (tournament_id, _config) = test_env.setup_payout_tournament(Some(admin.1.0), 2, 8, Some(100e8 as u64));
...
fn test_winner_basic_gtd_prizepool_surpassed() {
    ...
    let (tournament_id, _config) = test_env.setup_payout_tournament(Some(admin.1.0), 2, 8, Some(20e8 as u64));
